  ...
  app.post('/api/sales',auth,need('sales.create'),async(req,res)=>{
    ...
    try{
      ...
      for(const raw of lines){
        ...
        const effective=(await client.query('SELECT * FROM amaal_effective_variant_price($1,$2)',[v.id,b.customerId?((await client.query('SELECT customer_type FROM customers WHERE id=$1',[b.customerId])).rows[0]?.customer_type||'Retail'):'Retail'])).rows[0]; const price=raw.unitPrice===undefined?money(effective?.final_price??v.selling_price):money(raw.unitPrice); if(price<0)throw new Error('Unit price cannot be negative');
