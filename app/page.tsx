...
function CategoryCard({name,slug,index}:{name:string;slug:string;index:number}){
  return <Link className="category-card" href={`/categories/${slug}`}><div className="category-placeholder"><span>0{index+1}</span><strong>{name}</strong><small>PHOTO TO BE SUPPLIED</small></div><div className="category-copy"><h3>{name}</h3><p>Explore {name.toLowerCase()} at Amaal.</p></div></Link>
}
