...
if(next==='Dispatched'){const serialLines=(await client.query('SELECT ol.id,ol.quantity FROM order_lines ol JOIN product_variants v ON v.id=ol.variant_id WHERE ol.order_id=$1 AND v.serialized',[o.id])).rows;for(const line of serialLines){const assigned=(await client.query('SELECT count(*)::int c FROM order_serial_units WHERE order_line_id=$1',[line.id])).rows[0].c;if(assigned!==Number(line.quantity))throw new Error('Every serialized order line must have all physical units assigned before dispatch');}}
      ...
      if(next==='Cancelled'){if(o.fulfillment_status!=='Unfulfilled'&&o.status!=='Paid')throw new Error('Only unfulfilled orders can be cancelled');await releaseReservations(client,o.id,'Cancelled');const reservedUnits=(await client.query("SELECT su.id FROM serialized_units su JOIN order_serial_units os ON os.serialized_unit_id=su.id JOIN order_lines ol ON ol.id=os.order_line_id WHERE ol.order_id=$1 AND su.status='Reserved' FOR UPDATE",[o.id])).rows;for(const su of reservedUnits)await transitionSerializedUnit(client,{unitId:su.id,toStatus:'In Stock',actorId:req.user.id,reason:`Released physical unit because ${o.order_no} was cancelled`,sourceType:'OrderCancellation',sourceId:o.id});}
      if(next==='Delivered'){
      ...
      const fulStatus=['Processing','Packed','Ready for Dispatch','Dispatched','Delivered'].includes(next)?next:(next==='Cancelled'?'Cancelled':'Unfulfilled');const u=(await client.query('UPDATE orders SET status=$1,fulfillment_status=$2,updated_at=now(),cancelled_at=CASE WHEN $1=\'Cancelled\' THEN now() ELSE cancelled_at END,cancellation_reason=CASE WHEN $1=\'Cancelled\' THEN $4 ELSE cancellation_reason END WHERE id=$3 RETURNING *',[next,fulStatus,o.id,text(req.body?.reason||req.body?.notes)])).rows[0];await client.query('INSERT INTO order_status_history(order_id,status,actor_id,notes) VALUES($1,$2,$3,$4)',[o.id,next,req.user.id,text(req.body?.notes)||`Order moved to ${next}`]);if(o.customer_id)await client.query('INSERT INTO customer_notifications(customer_id,title,body,source_key) VALUES($1,$2,$3,$4) ON CONFLICT(source_key) WHERE source_key IS NOT NULL DO NOTHING',[o.customer_id,`Order ${o.order_no} updated`,`Your order is now ${next}.`,`order:${o.id}:status:${next}`]);await client.query('COMMIT');await audit(req.user,'ORDER_STATUS_CHANGED','Order',o.id,`Moved ${o.order_no} to ${next}`,o,u,req.req);res.json(await orderById(o.id));
    }catch(e){try{await client.query('ROLLBACK')}catch{}res.status(400).json({error:e.message})}finally{client.release()}
  });

  app.post('/api/orders/:id/cancel',auth,need('orders.manage'),async(req,res)=>{
    const client=await pool.connect();try{await client.query('BEGIN');const o=(await client.query("SELECT * FROM orders WHERE id=$1 FOR UPDATE",[req.params.id])).rows[0];if(!o)throw new Error('Order not found');if(!['Pending Payment','Paid'].includes(o.status))throw new Error('Only pending or paid orders can be cancelled');const reason=text(req.body?.reason);if(reason.length<3)throw new Error('A cancellation reason is required');await releaseReservations(client,o.id,'Cancelled');const reservedUnits=(await client.query("SELECT su.id FROM serialized_units su JOIN order_serial_units os ON os.serialized_unit_id=su.id JOIN order_lines ol ON ol.id=os.order_line_id WHERE ol.order_id=$1 AND su.status='Reserved' FOR UPDATE",[o.id])).rows;for(const su of reservedUnits)await transitionSerializedUnit(client,{unitId:su.id,toStatus:'In Stock',actorId:req.user.id,reason:`Released physical unit because ${o.order_no} was cancelled`,sourceType:'OrderCancellation',sourceId:o.id});const u=(await client.query("UPDATE orders SET status='Cancelled',fulfillment_status='Cancelled',cancelled_at=now(),cancellation_reason=$1,updated_at=now() WHERE id=$2 RETURNING *",[reason,o.id])).rows[0];await client.query('INSERT INTO order_status_history(order_id,status,actor_id,notes) VALUES($1,\'Cancelled\',$2,$3)',[o.id,req.user.id,reason]);await client.query('COMMIT');await audit(req.user,'ORDER_CANCELLED','Order',o.id,`Cancelled ${o.order_no}`,o,u,req.req);res.json(await orderById(o.id));}catch(e){try{await client.query('ROLLBACK')}catch{}res.status(400).json({error:e.message})}finally{client.release()}
  });
