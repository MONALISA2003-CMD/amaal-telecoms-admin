...
## PHASE 35 — Search-first phone catalogue homepage

### User direction
The phone catalogue homepage should follow a modern search-and-discovery organization similar to the supplied reference image: prominent search, horizontal brand navigation, filter sidebar, flat product grid, sorting, pagination, and access to the complete phone catalogue.

### Built
- `/phones` is now search-first.
- Prominent catalogue search supports model, series, brand, storage, RAM and network terms.
- Horizontal brand navigation with model counts.
- Flat all-model product grid instead of separate long brand sections.
- 24 / 48 / 96 / All page-size choices.
- Pagination keeps the full 156-model catalogue browsable without making the initial page excessively heavy.
- Search/filter changes reset to page 1.
- Previous/next pagination and accessible current-page state.
- Desktop filter sidebar and mobile filter drawer retained.
- Applied filter chips and clear-all behavior retained.
- Brand, family, network and storage filters retained.
- Product cards continue to open the model-specific `/phones/[slug]` page.
- Product imagery remains placeholders until Amaal supplies approved photography.

### UX principle
The phone catalogue is a public product-discovery catalogue, not an inventory screen. Every model remains discoverable; variants remain grouped beneath their model page.

### Source/reference
The supplied phone catalogue reference image was used as the visual/interaction reference for the search-first organization. No external product photography was introduced.

### Protected systems
No database reset. No schema reset. No backend replacement. No Business Admin Console changes.

### Validation status
The source update was written successfully. A complete TypeScript build requires the project's installed Next.js dependencies; the extracted Phase 34 archive does not include node_modules, so standalone `tsc` without dependencies cannot serve as a production-build result. Vercel remains the authoritative environment for the full Next.js build.
