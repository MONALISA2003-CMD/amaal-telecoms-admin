...
const checks=[
 ...
 ['sales transition to sold exists',/transitionSerializedUnit[\s\S]*toStatus:\s*'Sold'/.test(sales)&&/sale_serial_units/.test(sales)],
 ['delivery accepts reserved serialized units',/status!==['"]Reserved['"]/.test(delivery)&&/delivery_shipment_serial_units/.test(delivery)],
 ['returns preserve serialized unit',/serialized_unit_id/.test(returns)&&/transitionSerializedUnit/.test(returns)&&/toStatus:\s*'Returned'/.test(returns)],
