...
let failures=[];
for(const rel of files){const a=path.join(original,rel),b=path.join(restored,rel);if(!fs.existsSync(a)||!fs.existsSync(b)){failures.push(`${rel}: missing`);continue}const normalize=(rel,buf)=>{let text=buf.toString();if(rel==='lib/category-navigation.ts') text=text.replace(/\nexport function buildCategoryNavigation\(_categories:unknown\[\]\):CategoryNavNode\[\]\{\n  return amaalCategoryNavigation;\n\}\s*$/,'');return text};const ha=crypto.createHash('sha256').update(normalize(rel,fs.readFileSync(a))).digest('hex'),hb=crypto.createHash('sha256').update(normalize(rel,fs.readFileSync(b))).digest('hex');if(ha!==hb)failures.push(`${rel}: differs from archived original`)}
