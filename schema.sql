...
-- Backward-compatible Web & Hosting schema upgrades for databases created by earlier builds.
-- CREATE TABLE IF NOT EXISTS does not add columns to an already-existing table.
ALTER TABLE web_sites ADD COLUMN IF NOT EXISTS created_at timestamptz NOT NULL DEFAULT now();
ALTER TABLE web_sites ADD COLUMN IF NOT EXISTS updated_at timestamptz NOT NULL DEFAULT now();
ALTER TABLE web_pages ADD COLUMN IF NOT EXISTS created_at timestamptz NOT NULL DEFAULT now();
ALTER TABLE web_pages ADD COLUMN IF NOT EXISTS updated_at timestamptz NOT NULL DEFAULT now();
ALTER TABLE web_navigation ADD COLUMN IF NOT EXISTS created_at timestamptz NOT NULL DEFAULT now();
ALTER TABLE web_navigation ADD COLUMN IF NOT EXISTS updated_at timestamptz NOT NULL DEFAULT now();
ALTER TABLE web_banners ADD COLUMN IF NOT EXISTS created_at timestamptz NOT NULL DEFAULT now();
ALTER TABLE web_banners ADD COLUMN IF NOT EXISTS updated_at timestamptz NOT NULL DEFAULT now();
ALTER TABLE web_content_blocks ADD COLUMN IF NOT EXISTS updated_at timestamptz NOT NULL DEFAULT now();
ALTER TABLE web_domains ADD COLUMN IF NOT EXISTS created_at timestamptz NOT NULL DEFAULT now();
ALTER TABLE web_domains ADD COLUMN IF NOT EXISTS updated_at timestamptz NOT NULL DEFAULT now();
ALTER TABLE web_redirects ADD COLUMN IF NOT EXISTS created_at timestamptz NOT NULL DEFAULT now();
ALTER TABLE web_media ADD COLUMN IF NOT EXISTS created_at timestamptz NOT NULL DEFAULT now();
ALTER TABLE web_publish_releases ADD COLUMN IF NOT EXISTS created_at timestamptz NOT NULL DEFAULT now();
ALTER TABLE web_publish_queue ADD COLUMN IF NOT EXISTS created_at timestamptz NOT NULL DEFAULT now();
ALTER TABLE web_settings ADD COLUMN IF NOT EXISTS updated_at timestamptz NOT NULL DEFAULT now();
...
CREATE TABLE IF NOT EXISTS promotions(
 ...
 customer_type text NOT NULL DEFAULT 'All' CHECK(customer_type IN ('All','Retail','Wholesale','Corporate','VIP','Custom')),
...
ALTER TABLE promotions ADD COLUMN IF NOT EXISTS customer_type text NOT NULL DEFAULT 'All';
ALTER TABLE promotions ADD COLUMN IF NOT EXISTS min_quantity numeric(18,3) NOT NULL DEFAULT 1;
...
CREATE INDEX IF NOT EXISTS idx_promotion_approvals_promotion ON promotion_approvals(promotion_id,status,requested_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_promotion_redemptions_promotion ON promotion_redemptions(promotion_id,redeemed_at DESC);

CREATE OR REPLACE FUNCTION amaal_effective_variant_price_qty(p_variant_id uuid,p_customer_type text DEFAULT 'Retail',p_quantity numeric DEFAULT 1,p_coupon_code text DEFAULT NULL)
...
WITH normalized AS (
 SELECT CASE lower(coalesce(p_customer_type,'retail'))
   WHEN 'individual' THEN 'Retail' WHEN 'retail' THEN 'Retail'
   WHEN 'business' THEN 'Wholesale' WHEN 'wholesale' THEN 'Wholesale'
   WHEN 'corporate' THEN 'Corporate' WHEN 'vip' THEN 'VIP' WHEN 'custom' THEN 'Custom'
   ELSE 'Retail' END AS customer_type
), ctx AS (
 SELECT v.selling_price AS bp,
        COALESCE((SELECT i.price FROM price_list_items i JOIN price_lists l ON l.id=i.price_list_id
          CROSS JOIN normalized n WHERE i.variant_id=v.id AND l.status='Active' AND l.customer_type IN (n.customer_type,'Custom')
          AND (l.valid_from IS NULL OR l.valid_from<=now()) AND (l.valid_to IS NULL OR l.valid_to>now())
          ORDER BY CASE WHEN l.customer_type=n.customer_type THEN 0 ELSE 1 END,l.priority ASC,l.updated_at DESC LIMIT 1),v.selling_price) AS pp,
 ...
 SELECT pr.id,pr.name,pr.type,pr.value,pr.stackable
 FROM promotions pr CROSS JOIN ctx CROSS JOIN normalized n
 WHERE pr.status IN ('Active','Scheduled')
   AND (pr.starts_at IS NULL OR pr.starts_at<=now()) AND (pr.ends_at IS NULL OR pr.ends_at>now())
   AND (pr.customer_type='All' OR pr.customer_type=n.customer_type)
   AND pr.min_quantity<=GREATEST(1,coalesce(p_quantity,1))
   AND (pr.scope_type='All' OR (pr.scope_type='Product' AND EXISTS(SELECT 1 FROM promotion_products x WHERE x.promotion_id=pr.id AND x.product_id=ctx.product_id))
   ...
   AND (p_coupon_code IS NULL OR EXISTS(SELECT 1 FROM coupons c WHERE c.code=upper(trim(p_coupon_code)) AND c.promotion_id=pr.id AND c.status='Active'
       AND (c.starts_at IS NULL OR c.starts_at<=now()) AND (c.ends_at IS NULL OR c.ends_at>now())))
   AND (pr.max_uses IS NULL OR (SELECT count(*) FROM promotion_redemptions r WHERE r.promotion_id=pr.id)<pr.max_uses)
 ORDER BY CASE WHEN p_coupon_code IS NOT NULL THEN 0 ELSE 1 END,pr.priority ASC,pr.updated_at DESC LIMIT 1
), promo AS (SELECT * FROM candidate)
SELECT ctx.bp,ctx.pp,promo.id,promo.name,
 CASE WHEN promo.id IS NULL THEN 0 WHEN promo.type='Percentage' THEN round((ctx.pp*promo.value/100)::numeric,2) ELSE LEAST(ctx.pp,round(promo.value::numeric,2)) END,
 GREATEST(0,round((ctx.pp-CASE WHEN promo.id IS NULL THEN 0 WHEN promo.type='Percentage' THEN ctx.pp*promo.value/100 ELSE LEAST(ctx.pp,promo.value) END)::numeric,2))
FROM ctx LEFT JOIN promo ON true;
$$;

CREATE OR REPLACE FUNCTION amaal_effective_variant_price(p_variant_id uuid,p_customer_type text DEFAULT 'Retail')
RETURNS TABLE(base_price numeric,price_list_price numeric,promotion_id uuid,promotion_name text,discount_amount numeric,final_price numeric) LANGUAGE sql STABLE AS $$
 SELECT * FROM amaal_effective_variant_price_qty(p_variant_id,p_customer_type,1,NULL);
$$;
ALTER TABLE sales ADD COLUMN IF NOT EXISTS promotion_id uuid REFERENCES promotions(id) ON DELETE SET NULL;
...
CREATE INDEX IF NOT EXISTS idx_sales_promotion ON sales(promotion_id,created_at DESC);
CREATE INDEX IF NOT EXISTS idx_orders_promotion ON orders(promotion_id,created_at DESC);
-- Delivery & Logistics
