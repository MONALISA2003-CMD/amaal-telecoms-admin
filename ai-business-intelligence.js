...
export function registerAIBusinessIntelligence({app,auth,need,q,pool,audit,superAdminOnly}){
  ...
  async function chatSnapshot(start,end){
    ...
    const [topProducts,topCustomers,lowStock,openOrders,overdueCredit]=await Promise.all([
      ...
      q("SELECT p.name product_name,v.sku,COALESCE(sum(ib.on_hand),0)::numeric on_hand,COALESCE(MIN(r.reorder_point),5)::numeric reorder_point FROM inventory_balances ib JOIN product_variants v ON v.id=ib.variant_id JOIN products p ON p.id=v.product_id LEFT JOIN inventory_reorder_rules r ON r.variant_id=ib.variant_id AND r.location_id=ib.location_id AND r.enabled=true GROUP BY p.id,p.name,v.id,v.sku HAVING COALESCE(sum(ib.on_hand-ib.reserved),0)<=COALESCE(MIN(r.reorder_point),5) ORDER BY on_hand ASC LIMIT 20"),
