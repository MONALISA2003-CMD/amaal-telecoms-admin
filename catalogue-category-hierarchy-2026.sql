...
WHERE NOT EXISTS (SELECT 1 FROM product_categories WHERE slug='entertainment-audio-portable-speakers');
...
WHERE NOT EXISTS (SELECT 1 FROM product_categories WHERE slug='tablets');
INSERT INTO product_categories(name,slug,description,status,website_visibility)
SELECT 'Apple iPad','tablets-ipad','Apple iPad families and configurations.','Active','Published'
WHERE NOT EXISTS (SELECT 1 FROM product_categories WHERE slug='tablets-ipad');
INSERT INTO product_categories(name,slug,description,status,website_visibility)
SELECT 'Samsung Galaxy Tab','tablets-samsung-galaxy-tab','Samsung Galaxy Tab families and configurations.','Active','Published'
WHERE NOT EXISTS (SELECT 1 FROM product_categories WHERE slug='tablets-samsung-galaxy-tab');
UPDATE product_categories child SET parent_id=parent.id,status='Active',website_visibility='Published'
...
WHERE NOT EXISTS (SELECT 1 FROM product_categories WHERE slug='accessories');
INSERT INTO product_categories(name,slug,description,status,website_visibility)
