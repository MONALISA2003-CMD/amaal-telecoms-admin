...
export function POSWorkspace({ locations }: { locations: PosLocation[] }) {
  ...
  return <div className="posWorkspace">
    ...
    <section className="workspaceHero"><div><span className="eyebrow">Point of sale</span><h2>New sale</h2><p>Use the existing sales engine to price, validate stock and record the transaction. No local database is created.</p></div></section>
    {message && <div className="inlineMessage" role="status">{message}</div>}
    <div className="posGrid">
      <section className="panel"><div className="panelHeading"><div><h3>Product search</h3><p>Availability and pricing come from the existing engine.</p></div></div><div className="posControls"><select value={locationId} onChange={(event) => setLocationId(event.target.value)} aria-label="Selling location"><option value="">Select location</option>{locations.map((location) => <option key={location.id} value={location.id}>{location.name}{location.code ? ` · ${location.code}` : ''}</option>)}</select><div className="posSearch"><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') search(); }} placeholder="Search product, SKU or barcode" /><button onClick={search}>Search</button></div></div><div className="productGrid">{products.map((product) => <button className="productTile" key={product.variant_id} onClick={() => add(product)}><strong>{product.product_name}</strong><span>{product.variant_name || product.sku}</span><b>{money(product.selling_price)}</b><small>{Number(product.available).toLocaleString('en-UG')} available{product.serialized ? ' · Serial required' : ''}</small></button>)}{!products.length && <div className="emptyState">Search for a product to begin.</div>}</div></section>
