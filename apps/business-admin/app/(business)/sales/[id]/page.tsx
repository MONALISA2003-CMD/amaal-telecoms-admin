...
export default async function SaleDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="detailWorkspace">
    ...
    <div className="detailGrid">
      <section className="panel"><div className="panelHeading"><div><h3>Sale lines</h3><p>Products and amounts recorded by the existing sales engine.</p></div></div><div className="tableWrap"><table><thead><tr><th>Product</th><th>SKU</th><th>Qty</th><th>Unit price</th><th>Discount</th><th>Total</th></tr></thead><tbody>{sale.lines?.length ? sale.lines.map((line) => <tr key={line.id}><td>{line.product_name}</td><td>{line.sku}</td><td>{line.quantity}</td><td>{money(line.unit_price, sale.currency || 'UGX')}</td><td>{money(line.discount_amount, sale.currency || 'UGX')}</td><td>{money(line.line_total, sale.currency || 'UGX')}</td></tr>) : <tr><td colSpan={6}><div className="emptyState">No sale lines are available.</div></td></tr>}</tbody></table></div></section>
