...
export default async function ProductDetailPage({ params }: { params: Promise<Params> }) {
  ...
  return <div className="productDetailWorkspace">
    ...
    <div className="productDetailGrid">
      <section className="panel detailPanel"><div className="panelHeading"><div><h3>Commercial variants</h3><p>SKUs, pricing and variant-level identity from the existing catalogue engine.</p></div></div>{variants.length ? <div className="tableWrap"><table><thead><tr><th>SKU</th><th>Variant</th><th>Price</th><th>Wholesale</th><th>Inventory tracking</th><th>Status</th></tr></thead><tbody>{variants.map((v) => <tr key={v.id}><td><strong>{v.sku || '—'}</strong></td><td>{v.variant_name || 'Default'}{v.color ? ` · ${v.color}` : ''}{v.storage ? ` · ${v.storage}` : ''}</td><td>{money(v.selling_price)}</td><td>{money(v.wholesale_price)}</td><td>{v.track_inventory ? 'Tracked' : 'Not tracked'}</td><td><span className="catalogStatus">{v.status || '—'}</span></td></tr>)}</tbody></table></div> : <div className="detailEmpty"><Package size={23} /><strong>No variants returned</strong><span>The authoritative catalogue has not exposed any active variant records for this product.</span></div>}</section>
      <section className="panel detailPanel"><div className="panelHeading"><div><h3>Catalogue identity</h3><p>The business-facing metadata currently held by the engine.</p></div></div><dl className="identityList"><div><dt>Slug</dt><dd>{product.slug}</dd></div><div><dt>Product type</dt><dd>{product.product_type || '—'}</dd></div><div><dt>Featured</dt><dd>{product.featured ? 'Yes' : 'No'}</dd></div><div><dt>Tags</dt><dd>{productTags.length ? productTags.map((tag) => tag.name).join(', ') : '—'}</dd></div><div><dt>SEO title</dt><dd>{product.seo_title || '—'}</dd></div><div><dt>SEO description</dt><dd>{product.seo_description || '—'}</dd></div></dl></section>
    </div>
  </div>;
}
