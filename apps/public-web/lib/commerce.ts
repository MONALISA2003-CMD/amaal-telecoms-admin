...
async function cartRequest(path:string,init:RequestInit={}):Promise<ServerCart>{const headers=new Headers(init.headers);headers.set('Content-Type','application/json');headers.set('X-Amaal-Guest-Cart-Token',guestToken());const csrf=typeof document!=='undefined'?(document.cookie.match(/(?:^|; )amaal_customer_csrf=([^;]+)/)?.[1]||''):'';if(csrf)headers.set('X-Amaal-Customer-CSRF',decodeURIComponent(csrf));const token=typeof localStorage!=='undefined'?localStorage.getItem('amaal_customer_token_v1'):'';if(token)headers.set('X-Amaal-Customer-Token',token);const r=await fetch(`${apiBase()}${path}`,{...init,headers,credentials:'include',cache:'no-store'});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data?.error||'Shopping bag action failed.');return data;}
...
export async function mergeWishlistWithCustomer(){const ids=getWishlist().filter(x=>/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(x));if(!ids.length)return null;const headers=new Headers({'Content-Type':'application/json'});const token=typeof localStorage!=='undefined'?localStorage.getItem('amaal_customer_token_v1'):'';if(token)headers.set('X-Amaal-Customer-Token',token);const csrf=typeof document!=='undefined'?(document.cookie.match(/(?:^|; )amaal_customer_csrf=([^;]+)/)?.[1]||''):'';if(csrf)headers.set('X-Amaal-Customer-CSRF',decodeURIComponent(csrf));const r=await fetch(`${apiBase()}/api/public/account/wishlist/merge`,{method:'POST',headers,credentials:'include',cache:'no-store',body:JSON.stringify({productIds:ids})});return r.ok?r.json():null}
...
export function toggleWishlist(id:string){const s=new Set(getWishlist());const saved=!s.has(id);saved?s.add(id):s.delete(id);write(WISHLIST_KEY,[...s]);trackCommerce('wishlist',{productId:id,saved});syncWishlistServer(id,saved);return saved}
async function syncWishlistServer(productId:string,saved:boolean){try{const headers=new Headers({'Content-Type':'application/json'});const token=typeof localStorage!=='undefined'?localStorage.getItem('amaal_customer_token_v1'):'';if(token)headers.set('X-Amaal-Customer-Token',token);const csrf=typeof document!=='undefined'?(document.cookie.match(/(?:^|; )amaal_customer_csrf=([^;]+)/)?.[1]||''):'';if(csrf)headers.set('X-Amaal-Customer-CSRF',decodeURIComponent(csrf));await fetch(`${apiBase()}/api/public/account/wishlist`,{method:'POST',headers,credentials:'include',cache:'no-store',body:JSON.stringify({productId,saved})});}catch{}}
