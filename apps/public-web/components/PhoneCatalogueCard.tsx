  ...
  return <article className="phone-modern-card">
    <Link href={`/phones/${product.slug}`} className="phone-modern-card-link" aria-label={`View ${product.name}`}>
      <div className="phone-modern-media">
        {product.image ? <Image src={product.image} alt={product.name} fill sizes="(max-width: 600px) 50vw, (max-width: 1100px) 33vw, 25vw" className="phone-modern-product-image" /> : <div className="phone-model-placeholder"><span>AMAAL</span><strong>{product.name}</strong><small>Image coming soon</small></div>}
