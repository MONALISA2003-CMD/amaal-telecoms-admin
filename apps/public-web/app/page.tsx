...
function ProductTile({p}:{p:HomeProduct}){
  return <article className="home-product-card">
    <Link href={`/product/${p.slug}`} className="home-product-link">
      <div className="product-media">{p.images?.[0] ? <img src={p.images[0]} alt={p.name} loading="lazy"/> : <div className="product-placeholder"><span>AMAAL</span><strong>{p.name}</strong><small>Product image</small></div>}</div>
...
function CategoryCard({name,slug,index}:{name:string;slug:string;index:number}){
  return <Link className="category-card" href={`/categories/${slug}`}><div className="category-placeholder"><span>0{index+1}</span><strong>{name}</strong><small>Explore the collection</small></div><div className="category-copy"><h3>{name}</h3><p>Explore {name.toLowerCase()} at Amaal.</p></div></Link>
}
...
export default function Home(){
  return <main>
    ...
    <section className="lux-hero">
      ...
      <div className="hero-stage" aria-label="Featured Amaal technology"><div className="hero-ambient"/><div className="hero-asset hero-tv"><img src="/products/samsung-u8000f-tv.webp" alt="Samsung Smart TV"/></div><div className="hero-asset hero-phone hero-phone-one"><img src="/products/iphone17-pro-max.webp" alt="iPhone 17 Pro Max"/></div><div className="hero-asset hero-phone hero-phone-two"><img src="/products/galaxy-s26-ultra.webp" alt="Samsung Galaxy S26 Ultra"/></div><div className="hero-asset hero-sound"><img src="/products/samsung-b550-soundbar.webp" alt="Samsung soundbar"/></div><div className="hero-floor"/></div>
    </section>
    ...
    <section className="lifestyle-feature"><div className="lifestyle-copy"><p className="eyebrow">AMAAL EDIT</p><h2>Bring home experiences that matter.</h2><p>From brilliant screens to powerful sound, from the kitchen to the office — discover technology selected for the way you live.</p><Link className="button gold" href="/shop">Explore the collection <ArrowRight size={16}/></Link></div><div className="lifestyle-scene"><img className="scene-product scene-tv" src="/products/samsung-u8000f-tv.webp" alt="Samsung TV"/><img className="scene-product scene-sound" src="/products/samsung-b550-soundbar.webp" alt="Samsung soundbar"/></div></section>
    ...
    <section className="section brand-section"><div className="section-head"><div><p className="eyebrow">TRUSTED NAMES</p><h2>Shop by brand</h2><p className="section-intro">Recognisable brands, presented simply, with products selected for everyday life.</p></div><Link className="quiet-link" href="/brands">Explore brands <ArrowRight size={15}/></Link></div><AutoRail className="brand-rail" label="Shop by brand" speed={0.4}>{homeBrands.map((name,i)=><Link href={`/brands/${name.toLowerCase().replace(/[^a-z0-9]+/g,'-')}`} className="brand-card" key={`${name}-${i}`}><span className="brand-logo-placeholder">{name}</span></Link>)}</AutoRail></section>

    <section className="section weekly-section"><div className="weekly-heading"><div><p className="eyebrow">THIS WEEK</p><h2>Weekly deals</h2><p>Explore current Amaal offers across phones, entertainment and the home.</p></div><Link className="button gold" href="/deals">See all deals <ArrowRight size={16}/></Link></div><div className="weekly-grid"><Link href="/deals" className="weekly-card"><span>PHONE DEALS</span><h3>Upgrade your everyday.</h3><small>Explore current offers</small></Link><Link href="/deals" className="weekly-card dark"><span>HOME ENTERTAINMENT</span><h3>Make movie night better.</h3><small>Explore current offers</small></Link><Link href="/deals" className="weekly-card"><span>APPLIANCE DEALS</span><h3>Better home, better value.</h3><small>Explore current offers</small></Link></div></section>
