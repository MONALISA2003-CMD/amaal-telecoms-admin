-- Finance & Accounting additive structures.
-- Runtime bootstrap uses schema.sql; this file documents the finance module structures.
...
CREATE TABLE IF NOT EXISTS finance_expenses(
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(), expense_no text UNIQUE NOT NULL, expense_date date NOT NULL DEFAULT current_date, supplier_id uuid REFERENCES suppliers(id) ON DELETE SET NULL, customer_id uuid REFERENCES customers(id) ON DELETE SET NULL, account_id uuid NOT NULL REFERENCES finance_accounts(id), cash_account_id uuid REFERENCES finance_cash_accounts(id), amount numeric(18,2) NOT NULL CHECK(amount>0), tax_amount numeric(18,2) NOT NULL DEFAULT 0 CHECK(tax_amount>=0), method text NOT NULL DEFAULT 'Cash' CHECK(method IN ('Cash','Mobile Money','Card','Bank Transfer','Other')), reference text NOT NULL DEFAULT '', description text NOT NULL, status text NOT NULL DEFAULT 'Posted' CHECK(status IN ('Draft','Posted','Voided')), created_by uuid REFERENCES users(id) ON DELETE SET NULL, created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now()
);
