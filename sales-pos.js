...
export function registerSalesPos({app,auth,need,q,pool,audit,changeStock}){
  ...
  app.post('/api/sales',auth,need('sales.create'),async(req,res)=>{
    ...
    try{
      ...
      for(const x of prepared){
        ...
        if(x.v.serialized){await transitionSerializedUnit(client,{unitId:x.serials[0],toStatus:'Sold',actorId:req.user.id,reason:`Sold on ${sale.sale_no}`,sourceType:'Sale',sourceId:sale.id,soldAt:true,clearLocation:true});await client.query('INSERT INTO sale_serial_units(sale_line_id,serialized_unit_id) VALUES($1,$2)',[line.id,x.serials[0]])}
      }
  ...
  app.post('/api/sales/:id/void',auth,need('sales.void'),async(req,res)=>{
    const client=await pool.connect();try{await client.query('BEGIN');const s=(await client.query("SELECT * FROM sales WHERE id=$1 AND status IN ('Completed','Paid','Partially Paid') FOR UPDATE",[req.params.id])).rows[0];if(!s)throw new Error('Only operational sales can be voided');
      ...
      for(const l of lines){if(l.track_inventory)await changeStock(client,{variantId:l.variant_id,locationId:s.location_id,delta:Number(l.quantity),type:'SALE_VOID',actorId:req.user.id,referenceType:'Sale',referenceId:s.id,reason:`Voided sale ${s.sale_no}`,unitCost:Number(l.cost_price||0)});if(l.serialized){const unitIds=(await client.query("SELECT su.id FROM serialized_units su JOIN sale_serial_units ss ON ss.serialized_unit_id=su.id WHERE ss.sale_line_id=$1 AND su.status='Sold' FOR UPDATE",[l.id])).rows;for(const su of unitIds)await transitionSerializedUnit(client,{unitId:su.id,toStatus:'In Stock',locationId:s.location_id,actorId:req.user.id,reason:`Voided sale ${s.sale_no}; physical unit returned to stock`,sourceType:'SaleVoid',sourceId:s.id});}}
